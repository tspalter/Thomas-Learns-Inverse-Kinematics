...
static UINT indicators[] =
{
	...
	ID_INDICATOR_BASEROT, // added for putting in settings
	ID_INDICATOR_CAPS,
	ID_INDICATOR_NUM,
	ID_INDICATOR_SCRL,
};
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	lstBasicCommands.AddTail(ID_VIEW_GEOMETRY);
	...
	mOGLView.mPtrStatusBar = &m_wndStatusBar;
	mOGLView.Create(NULL, (LPCTSTR)"Render Window", WS_CHILD | WS_VISIBLE, CRect(1, 1, rect.right - 3, rect.bottom), this, 104); // - 60 bottom
	mOGLView.ShowWindow(TRUE);

	mOGLView.Invalidate(TRUE);
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	//  the CREATESTRUCT cs
	mClassName = AfxRegisterWndClass(NULL,
		(HCURSOR)mHArrow, (HBRUSH)::GetStockObject(DKGRAY_BRUSH), hicon); //m_HArrow
...
// OnHelpWhichOpenGL()
// Creates a dialog to show which version of OpenGL
...
void CMainFrame::OnPaint() {
	...
	mOGLView.DrawScene(TRUE);
}

//void CMainFrame::OnSize(UINT nType, int cx, int cy) {
//	mOGLView.SetWindowPos(&wndTopMost, 1, 1, cx - 3, cy - 20, SWP_NOZORDER); // -60 bottom
//	CFrameWnd::OnSize(nType, cx, cy);
//}

void CMainFrame::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
	mOGLView.HandleKeyDown(nChar);
